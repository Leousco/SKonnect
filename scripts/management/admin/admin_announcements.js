...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const catMeta = {
    event: { label: "Event", cls: "cat-event-pill" },
    program: { label: "Program", cls: "cat-program-pill" },
    meeting: { label: "Meeting", cls: "cat-meeting-pill" },
    notice: { label: "Notice", cls: "cat-notice-pill" },
    urgent: { label: "Urgent", cls: "cat-urgent-pill" },
  };
  ...
  catRadios.forEach((radio) => {
    radio.addEventListener("change", function () {
      const meta = catMeta[this.value];
      if (meta && previewCatPill) {
        previewCatPill.textContent = meta.label;
  ...
  const previewFeaturedBadge = document.getElementById(
  ...
  if (featuredCheckbox) {
    featuredCheckbox.addEventListener("change", function () {
      ...
      if (previewFeaturedBadge) {
        previewFeaturedBadge.style.display = this.checked
          ? "inline-flex"
          : "none";
      }
    });
  ...
  let attachments = [
    ...
    { name: "Scholarship_Guidelines_2026.pdf", size: "1.2 MB", type: "pdf" },
  ];
  ...
  if (attachDropZone) {
    attachDropZone.addEventListener("dragover", (e) => {
      e.preventDefault();
      attachDropZone.style.borderColor = "var(--ap-lt)";
    });
